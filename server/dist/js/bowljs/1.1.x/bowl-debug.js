...
!function (global, undefined) {
	...
	var config = {},
	    doc = global.document,
	    useInteractive = global.attachEvent && !(global.opera != null && global.opera.toString() === '[object Opera]');
	...
	// 移除前后空格
	function trim(str) {
		return str == null ? '' : String(str).replace(/^\s+/, '').replace(/\s+$/, '');
	}
	...
	function URL(url) {
		var div = createDivContainsA(url),
		    a = div.firstChild;
		a.href = a.href;
	...
	URL.prototype.toString = function () {
		return ensureSuffix(this.protocol, ':') + '//' + this.host + ensurePrefix(this.pathname, '/') + ensurePrefix(this.search, '?') + ensurePrefix(this.hash, '#');
	};
	...
	function idToURL(id, ref) {
		...
		if (canBeCached && idURLMapping[cacheKey]) {
			return idURLMapping[cacheKey];
		}
	...
	function parseDeps(code) {
		...
		// 粗略移除代码中的注释
		code = code.replace(/^\s*\/\*[\s\S]*?\*\/\s*$/mg, '') // 多行注释
		.replace(/^\s*\/\/.*$/mg, ''); // 单行注释
	...
	var dependentChain = function () {
		...
		return {
			// 添加依赖记录
			add: function add(moduleId, depId) {
			...
			// 获取依赖于指定模块的模块记录
			get: function get(depId) {
				return whichDepOnMe[depId];
			},

			// 清除依赖于某个模块的记录（当被依赖模块就绪时，即可清除）
			clear: function clear(depId) {
	...
	var taskManager = function () {
		...
		var preloadTask = {
			...
			isReady: function isReady() {
				return this._scripts === undefined;
			},
  ...
  */
	var require = global.require = function (ids, callback) {
		...
		taskManager.add(new Module(null, callback, unifyArray(ids)));
	};
  ...
  */
	...
	extend(Module.prototype, {
		...
		setId: function setId(id) {
			...
			if (t._id) {
				throw new Error('module id cannot be changed');
			}
			...
			var deps = t._deps;
			if (deps) {
				var readyStates = t._readyStates = {},
				    dep;
		...
		// 获取模块是否就绪
		isReady: function isReady() {
			return this._readyStates === undefined;
		},

		// 通知当前模块，其某个依赖模块已经就绪
		notifyReady: function notifyReady(depId) {
		...
		exports: function exports() {
			...
			if (!module) {
				...
				if (typeof t._factory === 'function') {
					module.exports = {};
					var myRequire = function myRequire(id) {
					...
					myRequire.async = function (ids, callback) {
						...
						taskManager.add(new Module(null, callback, unifyArray(ids), t._dirname));
					};
  ...
  */
	bowljs.config = function (newConfig) {
		...
		if (newConfig.libPath) {
			config.libPath = fixPath(newConfig.libPath);
		}
		if (newConfig.appPath) {
			config.appPath = fixPath(newConfig.appPath);
		}
		...
		if (newConfig.map) {
			config.map = (config.map || []).concat(newConfig.map);
		}
