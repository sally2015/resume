 ...
 * dom-attr - v1.1.0 (2015-03-27T12:31:06+0800)
...
var re_boolAttr = /^(?:checked|selected|async|autofocus|autoplay|controls|defer|disabled|hidden|ismap|loop|multiple|open|readonly|required|scoped)$/i;

// 获取属性值
function getNodeAttribute(node, name) { return node.getAttribute(name); }
// 设置属性值
function setNodeAttribute(node, name, value) { node.setAttribute(name, value); }
// 移除属性
function removeNodeAttribute(node, name) { node.removeAttribute(name); }
