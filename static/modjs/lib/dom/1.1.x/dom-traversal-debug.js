 ...
 * dom-traversal - v1.1.1 (2015-08-28T16:05:53+0800)
...
function filterBySelector(nodes, NodeList, selector) {
	return new NodeList(selector ? domHelper.matches(selector, nodes) : nodes);
}

// 排序、过滤节点
function sortAndFilter(nodes, position, NodeList, selector) {
	domHelper.uniqueSort(nodes);
...
function findNodesUntil(nodes, position, until, filter) {
	if (typeof until === 'string') {
		until = domHelper.querySelectorAll(until);
...
return {
	...
	shortcuts: {
			...
			if (first) {
				if (selector == null) {
					...
					if (isValidNode) {
						...
						result = domHelper.querySelectorAll(selector).indexOf(first);
					}
				} else {
					// 指定节点在当前集合中的位置
		...
		children: function(selector) {
			...
			if (this.length > 1) { domHelper.uniqueSort(result); }
