 ...
 * dom-event - v1.1.1 (2015-08-28T16:05:27+0800)
...
function dispatch(e) {
	...
	var listeners = listenerManager.get(node, eventType);
	if (listeners) {
		// 遍历回调函数
		listeners.forEach(function(listener) {
			...
			if (listener.delegator) {
				...
				while (temp && temp !== node) {
					if ( domHelper.canMatch(temp, listener.delegator) ) {
						thisObj = temp;
...
var listenerManager = (function() {
	...
	function bindDispatch(node, bindType, listenerSpace) {
		addEventListener(node, bindType, dispatch);
	}

	// 创建存放监听器的数据空间
	var listenerSpace = domData.createDataSpace({
		onClone: function(targetNode) {
	...
	return {
		...
		remove: function(node, trueType, bindType, handler) {
			...
			if (removeWay === 1) {
				...
				removeEventListener(node, bindType, dispatch);
			}
			...
			if (allTypes) {
				if (removeWay === 2) {
					allTypes.forEach(function(type) {
						removeEventListener(node, type, dispatch);
					});
					allTypes = null;
				}
			}
...
function on(node, types, handler, options) {
	...
	types.forEach(function(type) {
		listenerManager.add(node, type, base.customExtend({
			handler: handler,
			trueType: type
		}, options, {
			overwrite: false
...
function off(node, types, handler) {
	...
	if (types) {
		types.forEach(function(type) {
			listenerManager.remove(node, type, type, handler);
		});
	} else {
