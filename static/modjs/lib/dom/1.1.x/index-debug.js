 ...
 * dom - v1.1.1 (2015-10-10T17:46:55+0800)
...
function querySelector(selector, context) {
	if ( context != null && context.length && !domBase.isNode(context) && !domBase.isWindow(context) ) {
		...
		while (++i < len) { domHelper.querySelectorAll(selector, context[i], result); }
		if (len > 1) { domHelper.uniqueSort(result); }
		return result;
	} else {
		return domHelper.querySelectorAll(selector, context);
	}
}
...
var NodeList = base.createClass(function(nodes) {
	...
	filter: function(selector) {
		return new this.constructor(
			typeof selector === 'function' ?
				arrProto.filter.apply(this, arguments) : domHelper.matches(selector, this)
		);
	},
	...
	add: function(selector, context) {
		return new this.constructor(
			domHelper.uniqueSort( base.merge( this.toArray(), query(selector, context) ) )
		);
	},
